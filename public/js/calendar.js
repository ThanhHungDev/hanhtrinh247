...
function Calendar() {
    ...
    this.checkDateHaveEvent = function( year, month, day ){
        ...
        if(
            typeof this.data[year] != 'undefined' &&
    ...
    this.deleteEvent = function( day ){
        ...
        if(this.data){
            ...
            if( dateInEvent ){
                ...
                this.data[numberCurrentYear][numberCurrentMonth][numberCurrentDate] = undefined
            }
        }
        this.draw()
    }
            ...
            if( document.getElementById(this.idModalCalendar) ){
                ...
                if( headerText ){
                    if( dates[0] == dates[dates.length - 1]){
                        headerText.innerHTML = dates[0] + "月" + (this.selectMonth + 1) + "日(火)"
                    }else{
                        headerText.innerHTML = dates[0] + "月" + (this.selectMonth + 1) + "日(火) - "
                        + dates[dates.length - 1] + "月" + (this.selectMonth + 1) + "日(火)"
                    }                 
                }
            }
        }
        // console.log( " goto new modal ")
        // if( this.onNewModalPopup ){
        //     this.onNewModalPopup( dates )
        // }
    }
    this.updateModalPopup = function( dates ){
        ...
        // if( this.onUpdateModalPopup ){
        //     this.onUpdateModalPopup( dates )
        // }
    }
